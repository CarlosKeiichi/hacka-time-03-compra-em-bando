...
const CODIGO = "import { carregar } from \"@/lib/dados\";\n\nconst tabela = carregar(\"listas-compras-semana.csv\");\nif (!tabela) throw new Error(\"arquivo nao encontrado\");\n\n// Menor e maior \"preco_unitario_pago_brl\" de cada \"item\"\nconst faixas = new Map<string, number[]>();\nfor (const linha of tabela.linhas) {\n  const chave = linha[\"item\"];\n  const valor = Number(linha[\"preco_unitario_pago_brl\"]);\n  if (!Number.isFinite(valor)) continue;\n  faixas.set(chave, [...(faixas.get(chave) ?? []), valor]);\n}\n\nconst variacao = [...faixas.entries()]\n  .filter(([, vs]) => vs.length > 1)\n  .map(([chave, vs]) => {\n    const min = Math.min(...vs);\n    const max = Math.max(...vs);\n    return { chave, min, max, aumento: (max / min - 1) * 100 };\n  })\n  .sort((a, b) => b.aumento - a.aumento);\n";

export default function Guia() {
  const passos = verificar();
  const feitos = passos.filter((p) => p.feito).length;
  const exemplo = exemploPratico(
    "listas-compras-semana.csv",
    "item",
    "preco_unitario_pago_brl",
    "faixa",
  );
  ...
  return (
    <main>
      ...
      <header className="cabeca">
        <div className="env">
          <span className="crachá">Time 03 · Guia</span>
      ...
      <div className="env">
        ...
        <div className="bloco">
          ...
          <p className="prosa" style={{ marginTop: 10 }}>
            Este código responde: <b>qual a diferença de preço do mesmo item entre os restaurantes</b>. Ele já rodou no seu{" "}
            <span className="mono">listas-compras-semana.csv</span> — o resultado abaixo é do dado de vocês,
          ...
          <div className="aviso" style={{ marginTop: 20 }}>
            ...
            <p className="prosa" style={{ marginTop: 6 }}>
              ...
              <b>consolidar o volume do grupo e calcular a economia da negociação</b> — essa parte é o produto do desafio, e é ela que a banca
              vai olhar.
            </p>
          </div>
        </div>
